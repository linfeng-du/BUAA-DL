...
class Model():
    ...
    def gaussian_kernel(self, ksize, sigma=1):
    ...
    def _pool_layer(self, name, inp, ksize, stride, p=4, padding='SAME', mode='MAX'):
        assert mode in ['MAX', 'AVG', 'Lp'], 'the mode of pool must be MAX or AVG or Lp'
        if mode == 'MAX':
            ...
            x = tf.pow(inp, p)
            gauss_filter = tf.constant(self.gaussian_kernel(ksize), shape=[1, ksize, ksize, inp.shape[3]], dtype=tf.float32)
            x = tf.nn.conv2d(x, gauss_filter, strides=[1, stride, stride, 1],
                             padding=padding, data_format='NHWC')
            x = tf.pow(x, 1/p)
        return x
